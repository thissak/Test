...
void Texture::SetTextureFormat(int width, int height, uint32_t format, uint32_t type)
{
    ...
    GLenum imgFormat = GetImageFormat(m_format);
    glTexImage2D(GL_TEXTURE_2D, 0, m_format, m_width, m_height, 0, 
        imgFormat, m_type, nullptr);
}
...
void Texture::SetTextureFromImage(const Image *image)
{
    ...
    {
        m_type = GL_FLOAT;
        switch (image->GetBytePerChannel())
        {
            default: break;
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, m_format, m_width,
        m_height, 0, format, m_type, image->GetData() );
...
void CubeTexture::Init(int width, int height, int format, uint32_t type){
    ...
    for (uint32_t i = 0; i < 6; i++){
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, m_format, m_width,
            m_height, 0, imageFormat, m_type, nullptr);
    }
}
...
CubeTextureUPtr CubeTexture::CreateFromImages(const std::vector<Image*>& images){
    ...
    if (!texture->InitFromImages(images)){
        spdlog::error("fail to initFromImages");
        return nullptr;
    }
    return std::move(texture);  
}
