...
bool Context::Init()
{
    ...
    m_hdrMap = Texture::CreateFromImage(Image::Load("./img/christmas_photo_studio_07_2k.hdr").get());
    ...
    m_diffuseIrradianceProgram->SetUniform("cubMap", 0);
    m_hdrCubeMap->Bind();
...
void Context::Render() 
{
    ...
    m_sphericalMapProgram->SetUniform("transform", 
        projection * view * glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 2.0f)));
    m_sphericalMapProgram->SetUniform("tex", 0);
    m_hdrMap->Bind();
    m_box->Draw(m_sphericalMapProgram.get());
